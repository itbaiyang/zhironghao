...
articleCtrl.controller("ArticleListCtrl", function ($http, $scope, $rootScope, $location) {
    ...
    $scope.init = function () {
        $http({url: api_uri + "user/setting", method: "GET", params: $rootScope.login_user}).success(function (d) {
            console.log(d), 0 == d.returnCode ? $scope.company = d.result : console.log(d)
        }).error(function (d) {
            console.log(d)
        })
...
userCtrl.controller("UserCenterCtrl", function ($http, $scope, $rootScope, $timeout, $location) {
    ...
    $scope.init = function () {
        ...
        $http({url: api_uri + "loanApplication/list", method: "GET", params: m_params}).success(function (d) {
            console.log(d), 0 == d.returnCode && (0 == d.result.totalCount ? $scope.message = !1 : (result_list = result_list.concat(d.result.datas), $scope.result_list = result_list, $scope.nextPage = d.result.nextPage, $scope.pageNo = d.result.pageNo, $scope.totalCount = d.result.totalCount, angular.forEach($scope.result_list, function (data) {
    ...
    }, $scope.result_list = {result: {}, returnCode: 0}, $scope.init(), $scope.userDetail = function () {
...
myApp.run(["$location", "$rootScope", "$http", function ($location, $rootScope, $http) {
    ...
    $rootScope.wx_client = ua.indexOf("micromessenger") != -1, $rootScope.isIos = ua.indexOf("iphone") != -1 || ua.indexOf("ipad") != -1, $rootScope.wx_client && $http({
        ...
        var present_route = $location.$$path;
        $rootScope.check_user(), $rootScope.login_user ? (console.log(present_route), "/login" == present_route && $location.path("/user/center")) : no_check_route.indexOf(present_route) > -1 ? console.log(present_route) : present_route.indexOf("/article/show/") > -1 || ($rootScope.removeObject("login_user"), "/login" != present_route && $rootScope.putSessionObject("present_route", present_route), console.log(present_route), $location.path("/login"))
        ...
        $rootScope.login_user = $rootScope.getObject("login_user"), $http({
            ...
            return 0 == d.returnCode ? (console.log("login success"), !0) : ($rootScope.login_user = {}, $rootScope.removeObject("login_user"), $rootScope.present_route = $location.$$path, "/login" != $rootScope.present_route && $rootScope.putSessionObject("present_route", $rootScope.present_route), !1)
        }).error(function (d) {
            return !1
        })
