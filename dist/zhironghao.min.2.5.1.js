...
articleCtrl.controller("ArticleListCtrl", function ($http, $scope, $rootScope, $location) {
    $scope.list = function (pageNo, pageSize) {
        ...
        $http({url: api_uri + "financialProduct/list", method: "GET", params: m_params}).success(function (d) {
            0 == d.returnCode && ($scope.result_list = d.result.datas, $scope.totalCount = d.result.totalCount)
        }).error(function (d) {
            $location.path("/error")
        })
    ...
    $scope.init = function () {
        $scope.bt_show = 0, $http({
            ...
            if ($rootScope.shareBankname = d.result.bankname + "-", $rootScope.shareProductName = d.result.name, $rootScope.desc_detail = "额度：" + d.result.loanvalue + "  期限：" + d.result.loanlife + "  利息：" + d.result.rate + "%", 0 == d.returnCode) {
    ...
    $scope.apply = function () {
        $rootScope.present_route = $location.$$path, $rootScope.isNullOrEmpty(activityID) || $location.path("/article/apply/" + activityID)
    }
    ...
    $scope.init = function () {
        $http({url: api_uri + "user/setting", method: "GET", params: $rootScope.login_user}).success(function (d) {
            0 == d.returnCode && ($scope.company = d.result)
        }).error(function (d) {
        })
    }, $scope.init(), $scope.submit = function () {
        ...
        "undefined" == typeof m_params.companyName || "" == m_params.companyName ? ($scope.company.errorMsg = "公司名称不能为空", $timeout(function () {
...
userCtrl.controller("UserCenterCtrl", function ($http, $scope, $rootScope, $timeout, $location) {
    $scope.init = function () {
        null == $rootScope.login_user.userId || "" == $rootScope.login_user.userId ? $location.path("/login") : ($http({
            ...
            0 == d.returnCode ? ($scope.nickname = d.result.nickname, $scope.batting = d.result.batting, $scope.value = d.result.value, $scope.position = d.result.position, $scope.headImg = d.result.headImg, $scope.msgCount = d.result.msgCount) : ($rootScope.removeObject("login_user"), $location.path("/login")), $scope.init_role()
        }).error(function (d) {
        ...
        $http({url: api_uri + "loanApplication/myList", method: "GET", params: m_params}).success(function (d) {
            0 == d.returnCode && (0 == d.result.totalCount ? ($scope.message_myList = !1, 0 == $scope.message_myList && 0 == $scope.message_list ? $scope.message = !1 : $scope.message = !0) : ($scope.message_myList = !0, $scope.my_list = d.result.datas, $scope.myTotalCount = d.result.totalCount, angular.forEach($scope.my_list, function (data) {
                0 == $scope.type || 1 == $scope.type && (1 == data.status ? (data.jindu = "10", data.triangle = "8", data.textPosition = "2", data.progressText = "申请中", data.statusNextText = "约见中", data.progressNext = "审核中", data.progressTextNext = "开始约见", $scope.message = !0) : 2 == data.status ? (data.jindu = "50", data.triangle = "44", data.textPosition = "36", data.progressText = "约见中", data.statusNextText = "跟进中", data.progressNext = "审核中", data.progressTextNext = "继续跟进", $scope.message = !0) : 3 == data.status ? (data.jindu = "75", data.triangle = "66", data.textPosition = "58", data.progressText = "跟进中", data.statusNextText = "成功融资", data.progressNext = "审核中", data.progressTextNext = "完成贷款", $scope.message = !0) : 4 == data.status ? (data.jindu = "100", data.triangle = "86", data.textPosition = "76", data.progressText = "成功融资", data.progressNext = "已经完成", $scope.message = !0) : data.status == -1 && (data.jindu = "0", data.triangle = "20", data.textPosition = "5", data.progressText = "申请已经取消"))
            })))
        }).error(function (d) {
        })
    }, $scope.result_list = {
        ...
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $scope.alertText = "预计时间", $scope.alertText1 = "备注内容", $scope.alertText2 = "企业方需要补充一张个人征信表和法人的身份证证件"
    }, $scope.alert_come1 = function (status, id, days) {
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $scope.alertText = "延长时间", $scope.alertText1 = "说明原因", $scope.alertText2 = "企业方征信问题没有及时提交，请尽快提交到位"
        ...
        $.ajax({
            ...
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                0 == data.returnCode ? ($scope.alert = !1, $(".alertCenterSubmit").css("display", "block", "top", $(document).scrollTop()), $timeout(function () {
                    $(".alertCenterSubmit").css("display", "none")
                }, 2e3), $scope.$apply()) : console.log(data)
            },
            dataType: "json"
        })
    }
    ...
    $scope.list = function (pageNo, pageSize) {
        ...
        $http({url: api_uri + "zrh/message/listc", method: "GET", params: m_params}).success(function (d) {
            0 == d.returnCode && (0 == d.result.totalCount ? $scope.message_list = !1 : ($scope.message_list = d.result.datas, $scope.totalCount = d.result.totalCount))
        }).error(function (d) {
        })
    ...
    $scope.choiceStyle = $routeParams.defineId, $scope.init = function () {
        $scope.bt_show = 0, 1 == $routeParams.defineId ? $http({
            ...
            0 == d.returnCode && ($scope.company = d.result, 1 == $scope.company.status ? ($scope.company.jindu = "10", $scope.company.triangle = "8", $scope.company.textPosition = "2", $scope.company.progressText = "审核中", $scope.progressTextNext = "开始约见", $scope.message = !0) : 2 == $scope.company.status ? ($scope.company.jindu = "50", $scope.company.triangle = "44", $scope.company.textPosition = "36", $scope.company.progressText = "约见中", $scope.progressTextNext = "继续跟进", $scope.message = !0) : 3 == $scope.company.status ? ($scope.company.jindu = "75", $scope.company.triangle = "66", $scope.company.textPosition = "58", $scope.company.progressText = "跟进中", $scope.progressTextNext = "完成贷款", $scope.message = !0) : 4 == $scope.company.status ? ($scope.company.jindu = "100", $scope.company.triangle = "86", $scope.company.textPosition = "76", $scope.company.progressText = "成功融资", $scope.progressTextNext = "贷款结束", $scope.message = !0) : $scope.company.status == -1 && ($scope.company.jindu = "0", $scope.company.triangle = "20", $scope.company.textPosition = "5", $scope.company.progressText = "申请已经取消"))
        }).error(function (d) {
            $location.path("/error")
        }), $http({
            ...
            0 == d.returnCode && ($scope.company_basic = d.result, $scope.template_list = d.result.templateList, angular.forEach($scope.company_basic.templateList, function (data) {
            }))
        }).error(function (d) {
        })
        ...
        $http({
            ...
            0 == d.returnCode && $location.path("/user/center")
        }).error(function (d) {
            $location.path("/error")
        })
    }, $scope.alert = !1, $scope.alert_come = function (status, id, days) {
    ...
    }, $scope.expectDateBank = "", $scope.closeAlert = function (name, $event) {
        $scope.alert = !1, $scope.stopPropagation && $event.stopPropagation()
    }, $scope.sure = function () {
        var params = {
            userId: $rootScope.login_user.userId,
        ...
        $.ajax({
            ...
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                0 == data.returnCode && ($scope.alert = !1)
            },
            dataType: "json"
        })
    }
    ...
    $scope.init = function () {
        ...
        var params = $rootScope.login_user;
        params.key = $scope.update_user.key, params.value = $scope.update_user.value;
...
myApp.run(["$location", "$rootScope", "$http", "$routeParams", function ($location, $rootScope, $http, $routeParams) {
    ...
    $rootScope.wx_client = ua.indexOf("micromessenger") != -1, $rootScope.isIos = ua.indexOf("iphone") != -1 || ua.indexOf("ipad") != -1, $rootScope.wx_client && $http({
        ...
        id ? ($rootScope.title = $rootScope.shareBankname + $rootScope.shareProductName, $rootScope.desc = $rootScope.desc_detail) : url.indexOf("showActivity") ? ($rootScope.title = "直融号8月活动", $rootScope.desc = "千亿资金等你来拿") : ($rootScope.title = "直融号", $rootScope.desc = "打造企业最低融资成本");
        var strs = [];
        ...
        if ($rootScope.removeSessionObject("showID"), "/article/list" == present_route || present_route.indexOf("/article/show/") > -1 || "/article/showActivity" == present_route) {
            ...
            console.log(share), wx.ready(function () {
