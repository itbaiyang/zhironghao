...
articleCtrl.controller("ArticleListCtrl", function ($http, $scope, $rootScope, $location, $routeParams) {
    $scope.list = function (pageNo, pageSize) {
        ...
        $http({url: api_uri + "financialProduct/list", method: "GET", params: m_params}).success(function (d) {
            console.log(d), 0 == d.returnCode && ($scope.result_list = d.result.datas, $scope.totalCount = d.result.totalCount, wx.ready(function () {
                $rootScope.getUrl($location.absUrl(), $routeParams.id), console.log($routeParams.id, "xiwanga")
            }))
        }).error(function (d) {
            $location.path("/error")
        })
    ...
    $scope.init = function () {
        $scope.bt_show = 0, $http({
            ...
            if (console.log(d), $rootScope.shareBankname = d.result.bankname + "-", $rootScope.shareProductName = d.result.name, $rootScope.desc_detail = "额度：" + d.result.loanvalue + "  期限：" + d.result.loanlife + "  利息：" + d.result.rate + "%", console.log($rootScope.desc), console.log(d), wx.ready(function () {
                    $rootScope.getUrl($location.absUrl(), $routeParams.id), console.log($routeParams.id, "xiwanga")
                }), 0 == d.returnCode) {
                $scope.article_detail = d.result, $scope.feature_list = d.result.feature, $scope.apply_List = d.result.conditions, $scope.id = d.result.id, $scope.type = d.result.type, console.log($scope.type);
                var desc = "";
                $scope.article_detail.ratecap && $scope.article_detail.ratefloor && (desc += "利息率:" + $scope.article_detail.ratecap + "%~" + $scope.article_detail.ratefloor + "%\r\n"), $scope.article_detail.loanvalue && (desc += "贷款额度:" + $scope.article_detail.loanvalue + "万\r\n"), $scope.article_detail.loanlife && (desc += "贷款期限:" + $scope.article_detail.loanlife + "年")
            }
        }).error(function (d) {
        })
    }, $scope.init(), $scope.apply = function () {
        $rootScope.present_route = $location.$$path, $rootScope.isNullOrEmpty($scope.id) || $location.path("/article/apply/" + $scope.id + $scope.type)
    }
    ...
    $scope.apply = function (type) {
        $rootScope.present_route = $location.$$path, $rootScope.isNullOrEmpty(activityID) || $location.path("/article/apply/" + activityID + type)
    }, wx.ready(function () {
    ...
    console.log($routeParams.id), console.log($routeParams.type), $scope.type = $routeParams.type, $scope.init = function () {
        $http({url: api_uri + "user/setting", method: "GET", params: $rootScope.login_user}).success(function (d) {
            0 == d.returnCode && ($scope.company = d.result, $scope.savePic())
        }).error(function (d) {
        })
    }, $scope.init(), $scope.savePic = function () {
        $scope.img_list_id = [], $scope.img_list_card = [], $http({
        ...
        console.log(m_params), console.log(m_params1), 0 == $scope.type || 1 == $scope.type ? "undefined" == typeof m_params.companyName || "" == m_params.companyName ? ($scope.company.errorMsg = "公司名称不能为空", $timeout(function () {
            ...
            success: function (data, textStatus, jqXHR) {
                0 == data.returnCode ? ($(".alertApply").css("display", "block"), $timeout(function () {
            ...
            dataType: "json"
        }) : 2 == $scope.type && ("undefined" == typeof m_params1.linkman || "" == m_params1.linkman ? ($scope.company.errorMsg = "联系人不能为空", $timeout(function () {
            $scope.company.errorMsg = ""
        }, 2e3)) : "undefined" == typeof m_params1.mobile || "" == m_params1.mobile ? ($scope.company.errorMsg = "联系电话不能为空", $timeout(function () {
            $scope.company.errorMsg = ""
        }, 2e3)) : (console.log("af"), $.ajax({
            type: "POST",
            url: api_uri + "loanApplication/create",
            data: m_params1,
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                console.log(data, "success"), 0 == data.returnCode ? ($(".alertApply").css("display", "block"), $timeout(function () {
                    $(".alertApply").css("display", "none"), $location.path("/user/center")
                }, 2e3), $scope.$apply()) : 1001 == data.returnCode ? ($scope.company.errorMsg = "贵公司已经申请过此产品", $timeout(function () {
                    $scope.company.errorMsg = ""
                }, 2e3), $scope.$apply()) : 1004 == data.returnCode || $location.path("/login")
...
loginCtrl.controller("LoginCtrl", function ($http, $scope, $rootScope, $location, $timeout) {
    ...
    $scope.changeErrorMsg = function (msg) {
        ...
        $rootScope.isNullOrEmpty($scope.loginUser.mobile) ? ($scope.changeErrorMsg("手机号码不能为空"), $("#mobile").focus()) : $http({
            ...
            1001 == d.returnCode ? ($scope.enableMobile = !0, $scope.success_msg = "手机号输入正确") : ($scope.enableMobile = !1, $scope.changeErrorMsg("手机号未注册"))
        }).error(function (d) {
        })
...
registerCtrl.controller("RegStep1Ctrl", function ($http, $scope, $rootScope, $location, $timeout) {
    $scope.registerUser = {
        ...
        $rootScope.isNullOrEmpty($scope.registerUser.mobile) ? ($scope.changeErrorMsg("手机号码不能为空"), $("#mobile").focus()) : $http({
            ...
            0 == d.returnCode ? ($scope.enableMobile = !0, $scope.times(), $http({
                ...
                0 == d.returnCode ? $scope.changeErrorMsg("短信验证码已经发送到你的手机") : $scope.changeErrorMsg(d.returnCode)
            }).error(function (d) {
        ...
        $scope.isVerify && $http({
            ...
            0 == d.returnCode ? $location.path("/register/step2/" + $scope.registerUser.mobile + "/" + d.result) : $scope.changeErrorMsg(d.result)
        }).error(function (d) {
        })
    }
}), registerCtrl.controller("RegStep2Ctrl", function ($http, $scope, $rootScope, $location, $routeParams, $timeout) {
    var share_msg = $rootScope.getSessionObject("share");
    share_msg || (share_msg = {shareName: null, shareId: null}), $scope.registerUser = {
        ...
        $scope.registerUser.password == $scope.registerUser.validatePwd && reg_str.test($scope.registerUser.password) ? $http({
            ...
            0 == d.returnCode && (alert("注册成功"), $rootScope.putObject("login_mobile", $scope.registerUser.mobile), $http({
                ...
                0 == d.returnCode ? ($rootScope.login_user = {
                    userId: d.result.split("_")[0],
                    token: d.result.split("_")[1]
                }, $rootScope.putObject("login_user", $rootScope.login_user), $location.path("/article/list")) : $scope.changeErrorMsg(d.result)
            }).error(function (d) {
            }))
        }).error(function (d) {
    ...
    $scope.resetUser = {
        ...
        $rootScope.isNullOrEmpty($scope.resetUser.mobile) ? ($scope.changeErrorMsg("手机号码不能为空"), $("#mobile").focus()) : $http({
            ...
            1001 == d.returnCode ? ($scope.enableMobile = !0, $scope.times(), $http({
                ...
                0 == d.returnCode ? ($("#code").focus(), $scope.changeErrorMsg("短信验证码已经发送到你的手机")) : $scope.changeErrorMsg(d.result)
            }).error(function (d) {
            })) : 2102 == d.returnCode ? ($scope.enableMobile = !1, $scope.changeErrorMsg("手机号码错误")) : 0 == d.returnCode && ($scope.enableMobile = !1, $scope.changeErrorMsg("手机号码未注册"))
        }).error(function (d) {
        })
    }, $scope.changeCode = function () {
        ...
        $http({url: api_uri + "reg/reset", method: "POST", params: $scope.resetUser}).success(function (d) {
            0 == d.returnCode && (alert("重置密码成功"), $rootScope.putObject("login_mobile", $scope.resetUser.mobile), $http({
                ...
                0 == d.returnCode && ($rootScope.login_user = {
                    userId: d.result.split("_")[0],
                    token: d.result.split("_")[1]
                }, $rootScope.putObject("login_user", $rootScope.login_user), $location.path("/article/list"))
            }).error(function (d) {
            }))
        }).error(function (d) {
        })
...
userCtrl.controller("UserCenterCtrl", function ($http, $scope, $rootScope, $timeout, $location) {
    $scope.message = !1, $scope.msg_red_see = !1, $scope.init = function () {
        null == $rootScope.login_user.userId || "" == $rootScope.login_user.userId ? $location.path("/login") : ($http({
            url: api_uri + "user/center",
            method: "GET",
            params: $rootScope.login_user
        }).success(function (d) {
            console.log(d), 0 == d.returnCode ? ($scope.nickname = d.result.nickname, $scope.batting = d.result.batting, $scope.value = d.result.value, $scope.position = d.result.position, $scope.headImg = d.result.headImg, $scope.msgCount = d.result.msgCount, $scope.nickname || $location.path("/user/add_name")) : ($rootScope.removeObject("login_user"), $location.path("/login")), $scope.init_role()
        }).error(function (d) {
            $rootScope.removeObject("login_user"), $location.path("/login")
        }), $scope.list(1, 100), $scope.myList(1, 100))
    }, $scope.init_role = function () {
        $http({url: api_uri + "user/role", method: "GET", params: $rootScope.login_user}).success(function (d) {
            0 == d.returnCode && ($scope.role = d.result)
        }).error(function (d) {
        })
    }, $scope.list = function (pageNo, pageSize) {
        ...
        $http({url: api_uri + "loanApplication/list", method: "GET", params: m_params}).success(function (d) {
            console.log(d), 0 == d.returnCode && (0 == d.result.totalCount ? $scope.message_list = !1 : ($scope.message_list = !0, $scope.result_list = d.result.datas, $scope.totalCount = d.result.totalCount, angular.forEach($scope.result_list, function (data) {
                0 == $scope.type || 1 == $scope.type && (0 == data.status || 1 == data.status ? (data.jindu = "10", data.progressText = "准备中", $scope.message = !0) : 2 == data.status ? (data.jindu = "25", data.progressText = "下户", $scope.message = !0) : 3 == data.status ? (data.jindu = "40", data.progressText = "审批中", $scope.message = !0) : 4 == data.status ? (data.jindu = "55", data.progressText = "审批通过", $scope.message = !0) : 5 == data.status ? (data.jindu = "70", data.progressText = "开户", $scope.message = !0) : 6 == data.status ? (data.jindu = "85", data.progressText = "放款", $scope.message = !0) : 7 == data.status ? (data.jindu = "100", data.progressText = "完成融资", $scope.message = !0) : data.status == -1 && (data.jindu = "0", data.progressText = "申请已经取消"))
            })))
        }).error(function (d) {
        })
        ...
        $http({url: api_uri + "loanApplication/myList", method: "GET", params: m_params}).success(function (d) {
            console.log(d), 0 == d.returnCode && (0 == d.result.totalCount ? ($scope.message_myList = !1, 0 == $scope.message_myList && 0 == $scope.message_list ? $scope.message = !1 : $scope.message = !0) : ($scope.message_myList = !0, $scope.my_list = d.result.datas, $scope.myTotalCount = d.result.totalCount, angular.forEach($scope.my_list, function (data) {
                0 == $scope.type || 1 == $scope.type && (0 == data.status || 1 == data.status ? (data.jindu = "10", data.progressText = "准备中", data.statusNextText = "下户", data.progressNext = "审核中", data.progressTextNext = "开始下户", $scope.message = !0) : 2 == data.status ? (data.jindu = "25", data.progressText = "下户", data.statusNextText = "审批中", data.progressNext = "审核中", data.progressTextNext = "开始审批", $scope.message = !0) : 3 == data.status ? (data.jindu = "40", data.progressText = "审批中", data.statusNextText = "审批通过", data.progressNext = "审核中", data.progressTextNext = "审核通过", $scope.message = !0) : 4 == data.status ? (data.jindu = "55", data.progressText = "审批通过", data.statusNextText = "开户", data.progressNext = "审核中", data.progressTextNext = "马上开户", $scope.message = !0) : 5 == data.status ? (data.jindu = "70", data.progressText = "开户", data.statusNextText = "放款", data.progressNext = "审核中", data.progressTextNext = "开始放款", $scope.message = !0) : 6 == data.status ? (data.jindu = "85", data.progressText = "放款", data.statusNextText = "完成融资", data.progressNext = "审核中", data.progressTextNext = "成功融资", $scope.message = !0) : 7 == data.status ? (data.jindu = "100", data.progressText = "完成融资", data.statusNextText = "完成融资", data.progressNext = "审核中", data.progressTextNext = "完成融资", $scope.message = !0) : data.status == -1 && (data.jindu = "0", data.triangle = "20", data.textPosition = "5", data.progressText = "申请已经取消"))
            })))
        }).error(function (d) {
        })
    }, $scope.alert = !1, $scope.alert_come = function (status, id, days) {
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $scope.alertText = "预计时间", $scope.alertText2 = "如需备注请在次进行描述"
    }, $scope.alert_come1 = function (status, id, days) {
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $scope.alertText = "延长时间", $scope.alertText2 = "请说明原因（必填）"
    }, $scope.init(), $scope.userSetting = function () {
        $location.path("/user/setting/")
    }, $scope.userMessage = function () {
        $location.path("/user/message")
    }, $scope.my_apply = function (id, defineId) {
        $rootScope.isNullOrEmpty(id) || $location.path("/user/companyDetail/" + id + "/" + defineId)
        ...
        params.expectDateBank ? $.ajax({
            type: "POST",
            url: api_uri + "applyBankDeal/refer",
            data: params,
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                0 == data.returnCode ? ($scope.alert = !1, $(".alertCenterSubmit").css("display", "block"), $timeout(function () {
                    $(".alertCenterSubmit").css("display", "none")
                }, 2e3), $scope.list(1, 100), $scope.myList(1, 100), $scope.$apply()) : console.log(data)
            },
            dataType: "json"
        }) : ($scope.msg_red = "请输入预计时间", $scope.msg_red_see = !0, $timeout(function () {
            $scope.msg_red_see = !1
        }, 3e3))
    }
    ...
    $scope.list = function (pageNo, pageSize) {
        ...
        $http({url: api_uri + "zrh/message/listc", method: "GET", params: m_params}).success(function (d) {
            console.log(d), 0 == d.returnCode && (0 == d.result.totalCount ? $scope.message_list = !1 : ($scope.message_list = d.result.datas, $scope.totalCount = d.result.totalCount))
        }).error(function (d) {
        })
    }, $scope.list(1, 20), $scope.read_message = function (id, url) {
        var m_params = {userId: $rootScope.login_user.userId, token: $rootScope.login_user.token, id: id};
        $http({url: api_uri + "zrh/message/detailc", method: "GET", params: m_params}).success(function (d) {
            0 == d.returnCode && $scope.my_apply(url, 0)
        }).error(function (d) {
        })
    }, $scope.my_apply = function (urlDetail, defineId) {
        $rootScope.isNullOrEmpty(urlDetail) || $location.path(urlDetail + "/" + defineId)
    }
}), userCtrl.controller("CompanyDetailCtrl", function ($http, $scope, $rootScope, $location, $routeParams, $timeout) {
    $scope.msg_red_see = !1, $scope.choiceStyle = $routeParams.defineId, 0 == $scope.showImg, $scope.init = function () {
        1 == $routeParams.defineId ? $http({
            url: api_uri + "loanApplication/detail/" + $routeParams.id,
            method: "GET",
            params: $rootScope.login_user
        }).success(function (d) {
            console.log(d), 0 == d.returnCode && ($scope.company = d.result)
        }).error(function (d) {
            $location.path("/error")
        }) : $http({
            ...
            console.log(d), 0 == d.returnCode && ($scope.company = d.result, 0 == $scope.company.status || 1 == $scope.company.status ? ($scope.company.jindu = "10", $scope.company.progressText = "准备中", $scope.company.statusNextText = "下户", $scope.company.progressNext = "审核中", $scope.progressTextNext = "开始下户", $scope.company.message = !0) : 2 == $scope.company.status ? ($scope.company.jindu = "50", $scope.company.progressText = "下户", $scope.company.statusNextText = "审批中", $scope.company.progressNext = "审核中", $scope.progressTextNext = "开始审批", $scope.company.message = !0) : 3 == $scope.company.status ? ($scope.company.jindu = "75", $scope.company.progressText = "审批中", $scope.company.statusNextText = "审批通过", $scope.company.progressNext = "审核中", $scope.progressTextNext = "审核通过", $scope.company.message = !0) : 4 == $scope.company.status ? ($scope.company.jindu = "100", $scope.company.progressText = "审批通过", $scope.company.statusNextText = "开户", $scope.company.progressNext = "审核中", $scope.progressTextNext = "马上开户", $scope.company.message = !0) : 5 == $scope.company.status ? ($scope.company.jindu = "100", $scope.company.progressText = "开户", $scope.company.statusNextText = "放款", $scope.company.progressNext = "审核中", $scope.progressTextNext = "开始放款", $scope.message = !0) : 6 == $scope.company.status ? ($scope.company.jindu = "100", $scope.company.progressText = "放款", $scope.company.statusNextText = "完成融资", $scope.company.progressNext = "审核中", $scope.progressTextNext = "成功融资", $scope.company.message = !0) : 7 == $scope.company.status ? ($scope.company.jindu = "100", $scope.company.progressText = "放款", $scope.company.statusNextText = "完成融资", $scope.company.progressNext = "完成融资", $scope.progressTextNext = "成功融资", $scope.company.message = !0) : $scope.company.status == -1 && ($scope.company.jindu = "0", $scope.company.triangle = "20", $scope.company.textPosition = "5", $scope.progressText = "申请已经取消"))
        }).error(function (d) {
            $location.path("/error")
        }), $http({
            ...
            console.log(d), 0 == d.returnCode && ($scope.template_list = d.result.templateList, $scope.template_list.length > 0 ? 1 == $scope.showImg : 0 == $scope.showImg, $scope.company_basic = d.result)
        }).error(function (d) {
        })
        ...
        var m_params = {userId: $rootScope.login_user.userId, token: $rootScope.login_user.token, status: 0};
        $http({
            url: api_uri + "loanApplication/cancel/" + $routeParams.id,
            method: "GET",
            params: m_params
        }).success(function (d) {
            console.log(d), 0 == d.returnCode && (alert("取消成功"), $location.path("/user/center"))
        }).error(function (d) {
            $location.path("/error")
        })
    }, $scope.alert = !1, $scope.alert_come = function (status, id, days) {
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $(".alert").css("top", $(document).scrollTop()), $scope.alertText = "预计时间", $scope.alertText2 = "如需备注请在次进行描述"
    }, $scope.alert_come1 = function (status, id, days) {
        $scope.dateTime = days, $scope.alert = !0, $scope.applyId = id, $scope.status = status, $(".alert").css("top", $(document).scrollTop()), $scope.alertText = "延长时间", $scope.alertText2 = "请说明原因（必填）"
    }, $scope.closeAlert = function (name, $event) {
        $scope.alert = !1, $scope.stopPropagation && $event.stopPropagation()
    }, $scope.submit_status = function () {
        var params = {
        ...
        params.expectDateBank ? $.ajax({
            ...
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                0 == data.returnCode ? $scope.alert = !1 : console.log(data)
            },
            dataType: "json"
        }) : ($scope.msg_red = "请输入预计时间", $scope.msg_red_see = !0, $timeout(function () {
            $scope.msg_red_see = !1
    ...
    $scope.typeKey = $routeParams.id, console.log($routeParams.id), $scope.sure = function () {
        var params = {
            userId: $rootScope.login_user.userId,
            token: $rootScope.login_user.token,
            j: JSON.stringify({name: $scope.userName, position: $scope.position})
        };
        console.log(params), $.ajax({
            ...
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                console.log(data), 0 == data.returnCode ? $location.path("/user/setting") : console.log(data)
            },
            dataType: "json"
        }), $location.path("/user/setting")
    }
}), userCtrl.controller("AddMessageCtrl", ["$scope", "$rootScope", "$location", "$http", function ($scope, $rootScope, $location, $http) {
    $scope.sure = function () {
        var params = {
            userId: $rootScope.login_user.userId,
            token: $rootScope.login_user.token,
...
}]), api_uri = "http://api.supeiyunjing.com/", root_uri = "http://app.supeiyunjing.com/#", activityID = "71a28b4d3c60481ebfb6270eb27fde2c", templates_root = "templates/", deskey = "abc123.*abc123.*abc123.*abc123.*";
...
myApp.run(["$location", "$rootScope", "$http", "$routeParams", function ($location, $rootScope, $http, $routeParams) {
    ...
    $rootScope.wx_client = ua.indexOf("micromessenger") != -1, $rootScope.isIos = ua.indexOf("iphone") != -1 || ua.indexOf("ipad") != -1,
    $rootScope.wx_client && $http({
        ...
        id ? ($rootScope.title = $rootScope.shareBankname + $rootScope.shareProductName, $rootScope.desc = $rootScope.desc_detail) : url.indexOf("showActivity") > -1 ? ($rootScope.title = "直融号8月活动", $rootScope.desc = "千亿资金等你来拿") : ($rootScope.title = "直融号", $rootScope.desc = "打造企业最低融资成本");
        var strs = [];
        strs = url.split("?"), console.log(strs), $rootScope.login_user && ($rootScope.userId = $rootScope.login_user.userId), $routeParams.shareId && ($rootScope.shareId = $routeParams.shareId);
        var m_params = {url: strs[0], userId: $rootScope.userId, from: 0, shareId: $rootScope.shareId};
        $.ajax({
            type: "POST",
            ...
            traditional: !0,
            success: function (data, textStatus, jqXHR) {
                console.log(data), 0 == data.returnCode && ($rootScope.shareReturn = data.result, $rootScope.shareReturn.sn = data.result.sn, $rootScope.shareReturn.token = data.result.token, wx.onMenuShareAppMessage({
                    ...
                    success: function () {
                        ...
                        $http({url: api_uri + "wxShare/isShare", method: "GET", params: params}).success(function (d) {
                        })
                    }
                    ...
                    imgUrl: "http://app.supeiyunjing.com/img/share.png",
                    success: function () {
                        var params = {sn: $rootScope.shareReturn.sn, token: $rootScope.shareReturn.token, to: "QQ"};
                        $http({url: api_uri + "wxShare/isShare", method: "GET", params: params}).success(function (d) {
                        })
                    }
                }), wx.onMenuShareWeibo({
                    ...
                    imgUrl: "http://app.supeiyunjing.com/img/share.png",
                    success: function () {
                        var params = {sn: $rootScope.shareReturn.sn, token: $rootScope.shareReturn.token, to: "Weibo"};
                        $http({url: api_uri + "wxShare/isShare", method: "GET", params: params}).success(function (d) {
                        })
                    }
                }))
        ...
        if (openid) {
            ...
            var m_params = {userId: $rootScope.login_user.userId, token: $rootScope.login_user.token, openid: openid};
            $http({url: api_uri + "user/wxBind", method: "GET", params: m_params}).success(function (d) {
            })
        }
        if ($rootScope.removeSessionObject("showID"), "/article/list" == present_route || present_route.indexOf("/article/show/") > -1 || "/article/showActivity" == present_route) {
            var shareName = $location.search().share;
            if (console.log(shareName), shareName) {
                var shareId = $location.search().shareId, share = {shareName: shareName, shareId: shareId};
                $rootScope.putSessionObject("share", share)
            }
            console.log(share)
        } else"undefined" == typeof WeixinJSBridge ? document.addEventListener ? document.addEventListener("WeixinJSBridgeReady", onBridgeReady, !1) : document.attachEvent && (document.attachEvent("WeixinJSBridgeReady", onBridgeReady), document.attachEvent("onWeixinJSBridgeReady", onBridgeReady)) : onBridgeReady()
    }), $rootScope.$on("$routeChangeStart", function (event, current, previous) {
        var present_route = $location.$$path;
        $rootScope.check_user(), $rootScope.login_user || no_check_route.indexOf(present_route) > -1 || no_check_route.indexOf(present_route) <= -1 && present_route.indexOf("/article/show") > -1 || no_check_route.indexOf(present_route) <= -1 && present_route.indexOf("register/step2") > -1 || no_check_route.indexOf(present_route) <= -1 && present_route.indexOf("register/reset2") > -1 || ($rootScope.removeObject("login_user"), $rootScope.putSessionObject("present_route", present_route), $location.path("/login"))
    ...
    }).when("/user/companyDetail/:id/:defineId", {
